...
const filePathT2 = './Q22024/table2_gis_for_spouse_of_someone_receiving_oas_pension_april2024.csv';
const filePathT3 = './Q22024/table3_gis_for_spouse_of_someone_who_does_not_receive_oas_pension_april2024.csv';
...
function table1GISTest(filePath, se, tu, pe, years, iyears, age, date, verbose, outputFile){
    ...
    const calculate = (row) => {
        const date = new Date();
        const calculated = calculateTable1GIS(se, tu, pe, row.income_low, years, iyears, age, "2024-04-01", true)
...
// Output responses to a text file
function table2GISTest(filePath, se, tu, pe, years, iyears, age, date, verbose, outputFile){
  const columnHeaders = ["income_low","income_high", "GIS", "OASGIS65", "OASGIS75"]
  const startMessage = `Table 2 Test Start - ${age}`
  appendLineToFile(outputFile, startMessage)
  const calculate = (row) => {
      const date = new Date();
      const calculated = calculateTable2GIS(se, tu, pe, row.income_low, years, iyears, age, "2024-04-01", true)
      const output = parseFloat(calculated) === parseFloat(row.OASGIS65) ? ["T2 ", row.income_low, "passed", date] : {calculated, expected: row.OASGIS65}
      if(!(parseFloat(calculated) === parseFloat(age === 65? row.OASGIS65 : row.OASGIS75) && !verbose)){
          console.log(output)
          appendLineToFile(outputFile, JSON.stringify(output))
      }
...
// Output responses to a text file
function table3GISTest(filePath, se, tu, pe, years, iyears, age, date, verbose, outputFile){
  const columnHeaders = ["income_low","income_high", "GIS", "OASGIS65", "OASGIS75", "ALLOWANCE"]
  const startMessage = `Table 3 Test Start - ${age}`
  appendLineToFile(outputFile, startMessage)
  const calculate = (row) => {
      const date = new Date();
      const calculated = calculateTable3GIS(se, tu, pe, row.income_low, years, iyears, age, "2024-04-01", true)
      const output = parseFloat(calculated) === parseFloat(row.OASGIS65) ? ["T3 ", row.income_low, "passed", date] : {calculated, expected: row.OASGIS65}
      if(!(parseFloat(calculated) === parseFloat(age === 65? row.OASGIS65 : row.OASGIS75) && !verbose)){
          console.log(output)
          appendLineToFile(outputFile, JSON.stringify(output))
      }
